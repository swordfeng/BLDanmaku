...
class BLDanmaku;
class DanmakuModel :public QAbstractItemModel
{
	Q_OBJECT
public:
    ...
    virtual ~DanmakuModel();
	QList<Record> &getPool();
	void draw(QPainter *painter, double move);

    explicit DanmakuModel(BLDanmaku *danmakuItem);

private:
    DanmakuModelPrivate *const d_ptr;
    Q_DECLARE_PRIVATE(DanmakuModel);
